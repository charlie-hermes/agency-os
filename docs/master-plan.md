...
Paperclip `2026.720.0`, its versioned paths, package/route/reference checksums,
`paperclip.service` identity and hardening, private authenticated health shape,
and the current Buzz binary hash and collaboration command surface. A local
verifier rechecks those facts and fails closed on drift without credentials,
