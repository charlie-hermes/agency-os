...
class FleetInitializerTests(unittest.TestCase):
    ...
    def test_initializer_is_atomic_and_idempotent(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(first["account"]["client_brand_id"], "client_brand_fleet")
            self.assertEqual(first["enabled_modules"], ["ai_market_observatory", "brand_agent", "brand_twin", "client_portal", "content_engine", "controlled_actions"])
    ...
    def test_live_verifier_is_read_only_and_fails_closed_on_drift(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertEqual(result["enabled_modules"], ["ai_market_observatory", "brand_agent", "brand_twin", "client_portal", "content_engine", "controlled_actions"])
            connection = sqlite3.connect(database_path)
    ...
    def test_production_path_requires_and_checks_company_pin(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            company_pin.write_text(FLEET_COMPANY_ID + "\n")
            with patch.dict(
                "os.environ", {"FLEET_WORKOS_ORGANIZATION_ID": "org_fleet_live"}
            ), patch(
                "sys.argv",
